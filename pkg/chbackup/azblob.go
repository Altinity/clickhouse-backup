...
func (s *AzureBlob) GetFileReader(key string) (io.ReadCloser, error) {
	...
	r, err := blob.Download(ctx, 0, CountToEnd, BlobAccessConditions{}, false)
	if err != nil {
		return nil, err
	}
...
func (s *AzureBlob) PutFile(key string, r io.ReadCloser) error {
	...
	_, err := UploadStreamToBlockBlob(ctx, r, blob, UploadStreamToBlockBlobOptions{ BufferSize: bufferSize, MaxBuffers: maxBuffers })
	return err
}
...
func (s *AzureBlob) GetFile(key string) (RemoteFile, error) {
	...
	r, err := blob.GetProperties(ctx, BlobAccessConditions{})
	if err != nil {
...
func (s *AzureBlob) Walk(_ string, process func(r RemoteFile)) error {
	ctx := context.Background()
	opt := ListBlobsSegmentOptions{ /*Details: BlobListingDetails { Metadata: true }, */Prefix: s.Config.Path }
	mrk := Marker{}
