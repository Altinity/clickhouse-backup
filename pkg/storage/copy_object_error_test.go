...
func TestIsPermanentCopyObjectError(t *testing.T) {
	testcases := []struct {
		...
		{"s3 NoSuchBucket wrapped", errors.Wrapf(&smithy.GenericAPIError{Code: "NoSuchBucket", Message: "The specified bucket does not exist"}, "S3->CopyObject src -> dst return error"), true},
		{"s3 NoSuchKey", &smithy.GenericAPIError{Code: "NoSuchKey"}, true},
		{"s3 NotFound", &smithy.GenericAPIError{Code: "NotFound"}, true},
		{"s3 PermanentRedirect", &smithy.GenericAPIError{Code: "PermanentRedirect"}, true},
		...
		{"gcs 404", &googleapi.Error{Code: http.StatusNotFound}, true},
		...
		{"azure 404", &azcore.ResponseError{ErrorCode: "CopySourceNotFound", StatusCode: http.StatusNotFound}, true},
