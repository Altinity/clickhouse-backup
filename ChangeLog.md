# v2.8.2
BUG FIXES
- `list` and `tables` text output is now written to stdout as a single write, so a log line on stderr can no longer land in the middle of a backup row when both streams are merged (`docker exec`, `2>&1`), the same fix `json` / `yaml` / `csv` / `tsv` formats got in v2.8.1
- `restore` / `restore_remote` with `--restore-database-mapping` / `--restore-table-mapping` of tables on object disks (`s3`, `azure_blob_storage`, including `cache` and `encrypted` wrappers) no longer leaves the restored copy sharing object storage keys with the source table, so `DROP TABLE|DATABASE ... SYNC` of the copy no longer deletes the objects of the source (`Code: 499 ... The specified key does not exist (S3_ERROR)` on the source afterwards): the key rewrite was skipped when the disk endpoint key prefix contained the backup name (e.g. backup `regression` on the disk `https://s3/bucket/regression-test/live/`), and the metadata files were linked into `detached` / the table directory before the keys were rewritten, so an interrupted restore left the copy pointing to the source keys; the rewritten key now also carries a hash of the destination table (`<dir>_<backup_name>_<crc32(db.table)>/<object>`), so several copies of one backup restored next to each other never share objects, the source key of the copy is always derived from the backup, so the same local backup can be restored several times, and metadata files are written via temp file + rename, so a repeated restore never modifies the parts of an already restored table through their shared hardlink, fix [#1568](https://github.com/Altinity/clickhouse-backup/issues/1568)

# v2.8.1
NEW FEATURES
- add `clickhouse.drop_replica_if_exists` (env `CLICKHOUSE_DROP_REPLICA_IF_EXISTS`, CLI `--drop-replica-if-exists` for `restore`/`restore_remote`, also accepted as the `drop_replica_if_exists` query argument by `POST /backup/restore` and `POST /backup/restore_remote`) — when the replica entry `<replica_path>/replicas/<replica_name>` of a restored `Replicated*MergeTree` still exists in ZooKeeper but no local table uses that path, execute `SYSTEM DROP REPLICA '<replica_name>' FROM ZKPATH '<replica_path>'` and restore the table with the ORIGINAL replication path from the backup, instead of the default behavior which silently rebinds the table to `clickhouse.default_replica_path` / `clickhouse.default_replica_name`; requires ClickHouse 20.4+, the restore fails when the drop is impossible (older server, or a local table still occupies the path), the statement is executed without `ON CLUSTER` because the path and replica name are resolved with the macros of the current node; MUST stay `false` during a concurrent multi-replica restore, fix [#1162](https://github.com/Altinity/clickhouse-backup/issues/1162)
- add `--streaming` flag to `create_remote`, `restore_remote` and `watch`, `--watch-streaming` flag to `server` and the `streaming` query argument to `POST /backup/create_remote`, `POST /backup/restore_remote` and `POST /backup/watch` — process the backup table by table: `create_remote --streaming` runs freeze -> upload -> remove local copy per table, `restore_remote --streaming` runs download -> attach -> remove local copy per table, so only the tables in flight occupy local disk space and backups bigger than the free local space can be uploaded or restored; the local backup is not kept afterwards; interrupted runs are continued with `--resume` via the dedicated `create_upload_streaming.state2` / `download_restore_streaming.state2` state files (add these command names to `api.complete_resumable_after_restart_commands` to resume them after an API server restart); not available with `use_embedded_backup_restore: true` or `remote_storage: custom`, fix [#780](https://github.com/Altinity/clickhouse-backup/issues/780)
