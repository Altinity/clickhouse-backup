...
func resolveWaitForPrune(c *cli.Command, cfg *config.Config) (time.Duration, error) {
...
// casCommands returns the seven cas-* CLI subcommands. Root flags (--config,
// --env, --command-id) are persistent in urfave/cli v3 and reach every
// subcommand without being re-declared here.
func casCommands() []*cli.Command {
	return []*cli.Command{
		{
			Name:        "cas-upload",
			...
			Action: func(_ context.Context, c *cli.Command) error {
				...
				return b.CASUpload(c.Args().First(), c.Bool("skip-object-disks"), c.Bool("dry-run"), c.Bool("unlock"), version, commandIdFromCli(c), wait)
			},
			Flags: []cli.Flag{
				&cli.BoolFlag{
		...
		},
		{
			...
			Action: func(_ context.Context, c *cli.Command) error {
				...
				return b.CASDownload(c.Args().First(), c.String("tables"), c.StringSlice("partitions"), c.Bool("schema"), c.Bool("data"), version, commandIdFromCli(c))
			},
			Flags: []cli.Flag{
				&cli.StringFlag{
					Name:    "table",
		...
		},
		{
			...
			Flags: []cli.Flag{
				...
				&cli.BoolFlag{
					Name:    "configs",
					Aliases: []string{"restore-configs"},
				...
				&cli.BoolFlag{
					...
					Aliases: []string{"restore-named-collections"},
		...
		},
		{
			...
			Action: func(_ context.Context, c *cli.Command) error {
				...
				b := backup.NewBackuper(cfg)
				return b.CASDelete(c.Args().First(), commandIdFromCli(c), wait)
			},
			Flags: []cli.Flag{
				&cli.StringFlag{
					Name:  "wait-for-prune",
		...
		},
		{
			...
			Action: func(_ context.Context, c *cli.Command) error {
				b := backup.NewBackuper(config.GetConfigFromCli(c))
				return b.CASVerify(c.Args().First(), c.Bool("json"), commandIdFromCli(c))
			},
			Flags: []cli.Flag{
				&cli.BoolFlag{
					Name:  "json",
		...
		},
		{
			...
			Action: func(_ context.Context, c *cli.Command) error {
				b := backup.NewBackuper(config.GetConfigFromCli(c))
				return b.CASStatus(commandIdFromCli(c))
			},
		},
		{
			Name:        "cas-prune",
			...
			Action: func(_ context.Context, c *cli.Command) error {
				b := backup.NewBackuper(config.GetConfigFromCli(c))
				return b.CASPrune(c.Bool("dry-run"), c.String("grace-blob"), c.String("abandon-threshold"), c.Bool("unlock"), commandIdFromCli(c))
			},
			Flags: []cli.Flag{
				&cli.BoolFlag{
					Name:  "dry-run",
