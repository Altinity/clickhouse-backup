...
ln -nsfv /usr/lib/go-1.26/bin/go /usr/bin/go
...
# sudo -H bash -c 'export CLICKHOUSE_IMAGE=clickhouse/clickhouse-server; export CLICKHOUSE_VERSION=head; cd ./test/integration/; /root/go/bin/dlv --listen=127.0.0.1:40002 --headless=true --api-version=2 --accept-multiclient exec ./integration_test -- -test.timeout 30m -test.failfast -test.run "TestIntegrationEmbedded"'
...
# rootless
# cd /tmp/; wget https://go.dev/dl/go1.26.2.linux-arm64.tar.gz; tar -xzf go1.26.2.linux-arm64.tar.gz
