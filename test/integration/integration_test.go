...
func TestServerAPI(t *testing.T) {
	...
	out, err = dockerExecOut(
		"clickhouse",
		"bash", "-xe", "-c", "for i in {1..5}; do date; curl -sL -XPOST \"http://localhost:7171/backup/delete/local/backup_$i\"; curl -sL -XPOST \"http://localhost:7171/backup/download/backup_$i\"; sleep 2; curl -sL -XPOST \"http://localhost:7171/backup/restore/backup_$i?rm=1\"; sleep 3; done",
	)
	log.Debug(out)
	r.NoError(err)
	r.NotContains(out, "another operation is currently running")
