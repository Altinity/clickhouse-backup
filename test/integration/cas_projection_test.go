...
func TestCASUploadSkipObjectDisks(t *testing.T) {
	...
	r.NoError(env.dropDatabase(dbName, true))
	// Always-run cleanup via defer (NOT t.Cleanup): the body below has a
	// t.Skip path mid-flight, and t.Cleanup runs AFTER `defer env.Cleanup`
	// has already closed env.ch and returned the env to the pool — racing
	// with the next test acquiring that slot. defer runs LIFO before the
	// outer env.Cleanup defer, so it sees a still-live env.
	defer func() {
