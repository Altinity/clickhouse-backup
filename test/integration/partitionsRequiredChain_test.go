...
// partition 2. `readRequiredTableMetadata` used to trust that local file, so resolving the
...
func TestDownloadPartitionsRequiredChainS3(t *testing.T) {
	...
	makeStaleInc1 := func() string {
		...
		rawMeta, readErr := env.DockerExecOut("clickhouse-backup", "bash", "-ce", "cat "+incr1TableMeta)
		r.NoError(readErr, rawMeta)
		...
		r.NoError(json.Unmarshal([]byte(rawMeta), &staleTM), rawMeta)
	...
	// guard against passing trivially: 1_1_1_0 must come from the full backup, which is only reached by
	// walking past the filtered inc1, not from the big-files fallback or a hardlink of another local backup
	fullPartArchive := fullBackup + "/shadow/" + dbName + "/" + tableName + "/default_1_1_1_0.tar"
	r.Contains(downloadOut, fullPartArchive, "the 2-hop required-chain resolution did not reach the full backup")
	...
	// restore_remote (Download + Restore). The previous step resumed inc1 with --partitions=1 and
	// re-downloaded it, so the stale state has to be re-established before this flavour is meaningful.
	for _, backupName := range childrenFirst(backups) {
		env.DockerExecNoError(r, "clickhouse-backup", "clickhouse-backup", "-c", "/etc/clickhouse-backup/"+configFile, "delete", "--force", "local", backupName)
	}
	makeStaleInc1()
